...
const streams = [
  ...
  {
    id: 'frontier',
    // V6 CHANGE: renamed from 'Frontier technology & innovation'
...
const milestones = [
  ...
  {
    // V6 CHANGE: primaryStream command → intelligence; label GSK → R&D Risk Leadership
    id: 'gsk',
    years: '2025–present',
    markerYear: 2025.4,
    org: 'GSK',
    ...
    shift:
      'Applying the full convergence: AI/ML governance, epidemiology oversight, causal reasoning, and proactive risk leadership in a complex R&D environment.',
    proofs: [
...
const SVG_H            = 440;
const PLOT_LEFT        = 56;
const PLOT_RIGHT       = 960;   // streams end here = convergence threshold
const HORIZON_RIGHT    = 1085;  // V6: horizon zone extends to here
...
const CareerJourneyV6 = () => {
  ...
  const horizonMidX   = convergenceX + (HORIZON_RIGHT - convergenceX) / 2;
  ...
  return (
    ...
    >
      <style>{`
        ...
        .threshold-reveal {
          animation: thresholdReveal 1s ease 2s both;
        }
      `}</style>
      ...
      <header className="mx-auto max-w-5xl px-6 pt-14 pb-8">
        ...
        >
          ...
          experience converging on one discipline.
        </p>
      </header>
      ...
      <div className="mx-auto max-w-5xl px-4">
        <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full" style={{ overflow: 'visible' }}>
          ...
          {/* ── Gradient defs ───────────────────────────────── */}
          <defs>
            {/* Horizon zone fill: purple tint fading to transparent */}
            <linearGradient id="horizonFill" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%"   stopColor="#7c5cbf" stopOpacity="0.07" />
              <stop offset="100%" stopColor="#7c5cbf" stopOpacity="0"    />
            </linearGradient>
            {/* Forward projection line: fades out */}
            <linearGradient id="forwardFade" x1="0" y1="0" x2="1" y2="0">
              <stop offset="0%"   stopColor="#1e293b" stopOpacity="0.35" />
              <stop offset="100%" stopColor="#1e293b" stopOpacity="0.05" />
            </linearGradient>
            {/* Threshold line: subtle vertical gradient */}
            <linearGradient id="thresholdLine" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%"   stopColor="#7c5cbf" stopOpacity="0.08" />
              <stop offset="40%"  stopColor="#7c5cbf" stopOpacity="0.45" />
              <stop offset="100%" stopColor="#7c5cbf" stopOpacity="0.08" />
            </linearGradient>
          </defs>
          ...
          {sectorBands.map((band) => {
            ...
            return (
              <g key={band.id}>
                <rect
                  x={bx} y={6} width={bw} height={SVG_H - 48}
                  rx={5} fill={band.color} opacity={0.05}
                />
                <text
                  x={cx} y={SVG_H - 40}
                  textAnchor="middle"
                  fill={band.color} opacity={0.45}
            ...
            return (
              <g key={yr}>
                ...
                <text
                  x={tx} y={SVG_H - 8}
                  textAnchor="middle"
                  fill="#94a3b8" fontSize={10.5} fontWeight={600}
                  fontFamily="'Plus Jakarta Sans', sans-serif"
                >
                  {yr === 2026 ? 'Now' : yr}
                </text>
          ...
          {/* ── V6: Threshold + horizon zone ─────────────────
               Replaces the V5 terminus (dot, rings, arrow lines).
               Three elements: horizon fill, threshold line, forward projection.
          ─────────────────────────────────────────────────── */}

          {/* 1. Horizon zone — fading purple wash */}
          <rect
            x={convergenceX}
            y={12}
            width={HORIZON_RIGHT - convergenceX}
            height={SVG_H - 50}
            rx={4}
            fill="url(#horizonFill)"
            className="threshold-reveal"
          />

          {/* 2. Threshold vertical line — dashed, gradient-stroked via rect trick */}
          <line
            x1={convergenceX} y1={16}
            x2={convergenceX} y2={SVG_H - 38}
          ...
          {/* Threshold label — "NOW" below axis */}
          <text
            x={convergenceX} y={SVG_H - 8}
            textAnchor="middle"
            fill="#7c5cbf" opacity={0.55}
          ...
          {/* 3. Forward projection — fading dashed line from threshold */}
          <line
            x1={convergenceX + 2} y1={CONVERGENCE_Y}
            x2={HORIZON_RIGHT - 14} y2={CONVERGENCE_Y}
            stroke="url(#forwardFade)"
            strokeWidth={1.5}
            strokeDasharray="5 5"
            strokeLinecap="round"
            className="threshold-reveal"
          ...
          {/* Arrowhead at projection terminus */}
          <polygon
            points={`${HORIZON_RIGHT - 8},${CONVERGENCE_Y} ${HORIZON_RIGHT - 18},${CONVERGENCE_Y - 4} ${HORIZON_RIGHT - 18},${CONVERGENCE_Y + 4}`}
            fill="#1e293b"
            opacity={0.12}
            className="threshold-reveal"
          />

          {/* 4. Discipline label in horizon zone */}
          <text
            x={horizonMidX} y={CONVERGENCE_Y - 22}
            textAnchor="middle"
            fill="#7c5cbf" opacity={0.65}
            fontSize={9.5} fontWeight={700}
            fontFamily="'Plus Jakarta Sans', sans-serif"
          ...
          {/* Subtitle — hints at pioneering/leadership without specifying */}
          <text
            x={horizonMidX} y={CONVERGENCE_Y - 8}
            textAnchor="middle"
            ...
            className="threshold-reveal"
          >
            an emerging discipline, being led from the front
          </text>
          ...
          {education.map((ed) => {
            ...
            return (
              <g key={ed.id} className="dot-enter" style={{ animationDelay: '1.2s' }}>
                <line
                  x1={sx} y1={sy + 6}
                  x2={sx} y2={EDU_BASELINE - 18}
                  stroke="#cbd5e1" strokeWidth={0.7} strokeDasharray="2 3"
                />
                <rect
                  x={sx - 3.5} y={sy - 3.5}
                  width={7} height={7} rx={1.5}
                  fill="#faf9f7"
                  stroke={stream.color} strokeWidth={1.5}
                  transform={`rotate(45, ${sx}, ${sy})`}
                />
                <text
                  x={sx} y={EDU_BASELINE - 6}
                  textAnchor="middle"
                  fill="#94a3b8" fontSize={8.5} fontWeight={600}
                  fontFamily="'Plus Jakarta Sans', sans-serif"
                >
                  {ed.label}
                </text>
                <text
                  x={sx} y={EDU_BASELINE + 5}
                  textAnchor="middle"
                  fill="#cbd5e1" fontSize={7.5} fontWeight={500}
                  fontFamily="'Plus Jakarta Sans', sans-serif"
                >
                  {ed.institution}
                </text>
      ...
      <div className="mx-auto max-w-5xl px-6 py-6" style={{ minHeight: 180 }}>
        ...
        >
          ...
          <p className="mb-4" style={{ fontSize: 15, lineHeight: 1.75, color: '#64748b' }}>
            {active.shift}
          </p>
      ...
      {/* ── V6: Closing statement — emerging discipline ────── */}
      {/*
        CHANGED from V5: was personal philosophy / rhetorical question structure.
        V6 frames an emerging professional discipline — causal decision intelligence —
        and hints at pioneering/leadership-tier roles without specifying a destination.
      */}
      <section className="mx-auto max-w-3xl px-6 py-10">
        ...
        >
          ...
          >
            Intelligence-led decision-making — built on causal reasoning, anticipatory analytics,
            and systems thinking — is becoming the defining capability of organisations that lead,
            rather than react.
          </p>

          <p
            style={{ fontSize: 14, lineHeight: 1.85, color: '#64748b', marginBottom: 14 }}
          >
            Most organisations still treat risk management, intelligence, and data capability as
            separate functions. The emerging frontier is their integration: building the causal
            decision infrastructure that lets organisations see further, reason more rigorously,
            and act with greater confidence under uncertainty. This is not a mature discipline
            with established playbooks — it is being constructed, now, in the organisations
          ...
          <p
            style={{ fontSize: 14, lineHeight: 1.85, color: '#64748b', marginBottom: 14 }}
          >
            The through-line of this career has been an accidental apprenticeship in exactly that
            work. Command decisions at operational tempo. Intelligence operations that turned raw
            signal into foresight. Data platforms that embedded analytical capability at scale.
            Causal models that moved risk from reporting to anticipation. Each chapter added a
            layer of capability; the convergence is the point at which those layers become
          ...
          >
            The next chapter is about leading where this discipline is being built — in the
            functions, organisations, and teams pioneering what genuinely intelligent
            decision-making looks like in practice.
          </p>
        </div>
      </section>
...
export default CareerJourneyV6;
