...
const milestones = [
  {
    ...
    proofs: [
      'Maintained full operational readiness of the platoon, ensuring the unit was deployable at short notice',
      'Built a cohesive team across vehicle maintenance, welfare and military training, sustaining output through personnel transitions',
    ],
  ...
  },
  {
    ...
    proofs: [
      'Directed security force responses to high-threat incidents, directly reducing risk to personnel and public across the Province',
      'Intelligence analysis and inter-agency coordination with PSNI enabled faster, better-informed threat response',
    ],
  ...
  },
  {
    ...
    org: 'British Army',
    sector: 'Defence',
    title: 'Operations Officer — Defence School of Electronic and Mechanical Engineering',
    ...
    proofs: [
      'Delivered a 5,000-person relocation from Berkshire to Wiltshire without disrupting training output or operational continuity',
      'Merged two career training pipelines into one, reducing training duration and cost while meeting future military requirements',
      'Passed all OFSTED and MOD assurance audits, de-risking the programme for senior leadership',
    ],
    primaryStream: 'command',
  ...
  },
  {
    ...
    org: 'British Army',
    sector: 'Defence',
    title: 'Head of Engineering — 1 Regiment RLC, including operational deployment to Afghanistan',
    ...
    proofs: [
      'Returned 2,500 vehicles to operational status during the Afghanistan drawdown, enabling the UK to meet its withdrawal timeline',
      'Relocated an engineering workshop from Germany to Oxfordshire with minimal disruption, sustaining output throughout a year-long transition',
      'Achieved highest equipment availability in the region through lean process redesign, directly improving operational capability',
    ],
    primaryStream: 'command',
    activeStreams: ['command', 'systems'],
  ...
  },
  {
    ...
    proofs: [
      'Delivered a $30m cross-continental portfolio on time and within budget, accelerating genomic data flow into drug discovery',
      'Negotiated $350k in annual supplier savings, improving contract efficiency by 10%',
      'Enabled seamless coordination across four continents and multiple therapeutic areas, reducing bottlenecks in the sample-to-data pipeline',
    ],
    primaryStream: 'frontier',
    activeStreams: ['command', 'frontier'],
    label: 'Genomics Programme Mgr',
    labelBelow: true,
  },
  {
    id: 'hie',
    ...
    proofs: [
      'Delivered the first implementation of pharmacogenetics in NHS primary care, improving prescribing safety for patients',
      'Enabled the first use of AI on real patient data for rare disease diagnosis, shortening time to treatment for underserved patient groups',
      'Authored a commercial strategy that generated £3m in projected new income, transforming the organisation\'s funding model',
    ],
    primaryStream: 'frontier',
    activeStreams: ['frontier', 'intelligence'],
    label: 'Innovation Consultant',
    labelBelow: true,
  },
  {
    id: 'santander-ai',
    ...
    proofs: [
      'Improved data linkage quality from 83% to 98%, enabling reliable customer intelligence across the ring-fenced bank for the first time',
      'Freed 600 hours annually through RPA and cut process errors by 20%, redirecting capacity into higher-value analytical work',
      'Delivered ML-based pricing optimisation that generated bespoke rates for every customer, driving commercial performance',
    ],
    primaryStream: 'frontier',
    activeStreams: ['frontier', 'systems', 'command'],
  ...
  },
  {
    ...
    org: 'Santander UK',
    sector: 'Banking',
    title: 'Head of Financial Crime Customer 360 Data',
    ...
    proofs: [
      'Improved financial crime detection accuracy by 15%, strengthening the bank\'s regulatory position and reducing exposure',
      'Reduced false positives by 10%, improving customer experience and freeing investigator capacity for genuine threats',
    ],
    primaryStream: 'systems',
    activeStreams: ['systems', 'intelligence'],
    label: 'Financial Crime Data',
    labelBelow: false,
  },
  {
    id: 'santander-res',
    ...
    proofs: [
      'Reduced time to risk mitigation by 30%, enabling the bank to respond to emerging threats before they materialised',
      'Built a causal model for predicting systemic risks, giving senior leadership forward-looking visibility they previously lacked',
      'Deployed GenAI-driven intelligence summaries, cutting the time to produce risk briefings from days to hours',
      'Created executive dashboards that gave three lines of defence real-time visibility into change risk across the bank',
    ],
    primaryStream: 'intelligence',
    activeStreams: ['intelligence', 'systems', 'frontier', 'command'],
    label: 'Risk Intel & Governance',
    labelBelow: false,
  },
  {
    id: 'gsk',
    ...
    proofs: [
      'Providing risk leadership across AI/ML, Development Science, and Epidemiology — ensuring emerging capabilities are governed without slowing innovation',
      'Embedding anticipatory risk frameworks that help Development Science teams identify and address risks earlier in the pipeline',
    ],
    primaryStream: 'frontier',
    activeStreams: ['intelligence', 'systems', 'frontier', 'command'],
    isCurrent: true,
    label: 'Director, Risk Mgmt',
    labelBelow: true,
  },
];
...
const education = [
  { id: 'beng',      year: 2007,   label: 'BEng Mechanical Engineering',        institution: 'University of Nottingham',    stream: 'systems' },
  { id: 'sandhurst', year: 2009.5, label: 'Officer Training',                    institution: 'RMA Sandhurst',               stream: 'command' },
  { id: 'msc',       year: 2014,   label: 'MSc Information Capability Mgmt',     institution: 'Cranfield University',         stream: 'frontier' },
  { id: 'bsc',       year: 2017,   label: 'BSc Intelligence & Security',         institution: 'Staffordshire University',     stream: 'intelligence' },
  { id: 'mba',       year: 2020.5, label: 'MBA (Distinction)',                   institution: 'Warwick Business School',      stream: 'command' },
];
...
const SVG_W         = 1100;
const SVG_H         = 475;    // V6.5: extended for education row below axis
const PLOT_LEFT     = 56;
const CONVERGE_X    = 880;
...
// V6.5: Fixed axis positions (decoupled from SVG_H so education row can sit below)
const BAND_HEIGHT   = 392;    // sector band rect height
const BAND_LABEL_Y  = 400;    // sector name labels
const GRID_BOTTOM   = 406;    // vertical gridline bottom
const TICK_TOP      = 410;    // axis tick top
const TICK_BOTTOM   = 418;    // axis tick bottom
const YEAR_LABEL_Y  = 432;    // year number labels
const THRESHOLD_BOT = 402;    // threshold line bottom
const HORIZON_H     = 390;    // horizon wash height
const EDU_ROW_Y     = 455;    // education row label baseline
    ...
    <div style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", background: '#faf9f7', color: '#1e293b', minHeight: '100vh' }}>
      ...
      <header className="mx-auto max-w-5xl px-6 pt-14 pb-8">
        ...
        <p className="mt-4" style={{ fontSize: 15, lineHeight: 1.75, color: '#64748b', maxWidth: 600 }}>
          A career built across defence, biopharma, healthcare, and banking — four streams of experience converging towards better decision-making in complex technical environments.
        </p>
      </header>
      ...
      <div className="mx-auto max-w-5xl px-4">
        <svg viewBox={`0 0 ${SVG_W} ${SVG_H}`} className="w-full" style={{ overflow: 'visible' }}>
          ...
          {sectorBands.map((band) => {
            ...
            return (
              <g key={band.id}>
                <rect x={bx} y={6} width={bw} height={BAND_HEIGHT} rx={5} fill={band.color} opacity={0.05} />
                <text x={bx + bw / 2} y={BAND_LABEL_Y} textAnchor="middle" fill={band.color} opacity={0.45} fontSize={9.5} fontWeight={700} fontFamily="'Plus Jakarta Sans', sans-serif" letterSpacing="0.1em">
          ...
          {TICK_YEARS.map((yr) => {
            ...
            return (
              <g key={yr}>
                <line x1={tx} y1={TICK_TOP} x2={tx} y2={TICK_BOTTOM} stroke="#cbd5e1" strokeWidth={1.2} />
                <text x={tx} y={YEAR_LABEL_Y} textAnchor="middle" fill="#94a3b8" fontSize={10.5} fontWeight={600} fontFamily="'Plus Jakarta Sans', sans-serif">
                  {yr === 2026 ? 'Now' : yr}
                </text>
                <line x1={tx} y1={12} x2={tx} y2={GRID_BOTTOM} stroke="#e2e8f0" strokeWidth={0.5} />
              </g>
            );
          })}
          ...
          <rect
            x={CONVERGE_X} y={12}
            width={HORIZON_RIGHT - CONVERGE_X + 10} height={HORIZON_H}
          ...
          <line
            x1={CONVERGE_X} y1={16}
            x2={CONVERGE_X} y2={THRESHOLD_BOT}
          ...
          */}
          ...
          <text
            ...
            className="merge-reveal"
          >
            where intelligence, systems, and judgement meet
          </text>
          ...
          {/* Education row — dedicated line below axis */}
          {/* Separator */}
          <line
            x1={PLOT_LEFT} y1={EDU_ROW_Y - 18}
            x2={CONVERGE_X} y2={EDU_ROW_Y - 18}
            stroke="#e2e8f0" strokeWidth={0.5}
          />
          {/* Row label */}
          <text
            x={PLOT_LEFT - 2} y={EDU_ROW_Y + 2}
            textAnchor="end"
            fill="#cbd5e1" fontSize={7} fontWeight={700}
            fontFamily="'Plus Jakarta Sans', sans-serif"
            letterSpacing="0.1em"
          >
            EDUCATION
          ...
          {education.map((ed) => {
            ...
            return (
              <g key={ed.id} className="dot-enter" style={{ animationDelay: '1.2s' }}>
                ...
                <rect
                  x={sx - 3} y={EDU_ROW_Y - 3}
                  width={6} height={6} rx={1}
                  fill={stream.color} opacity={0.35}
                  transform={`rotate(45, ${sx}, ${EDU_ROW_Y})`}
                />
                {/* Degree label */}
                <text
                  x={sx} y={EDU_ROW_Y + 14}
                ...
                <text
                  x={sx} y={EDU_ROW_Y + 23}
                  textAnchor="middle"
                  fill="#94a3b8" fontSize={6.5} fontWeight={500}
                  fontFamily="'Plus Jakarta Sans', sans-serif"
                >
                  {ed.institution}
                </text>
      ...
      <section className="mx-auto max-w-3xl px-6 py-10">
        <div className="rounded-xl border px-8 py-8" style={{ borderColor: '#e2e8f0', background: '#f8fafc' }}>
          ...
          <p style={{ fontFamily: "'Libre Baskerville', Georgia, serif", fontSize: 'clamp(0.95rem, 2.1vw, 1.15rem)', fontWeight: 700, lineHeight: 1.65, color: '#1e293b', marginBottom: 20 }}>
            Decision-ready intelligence — built on causal reasoning, anticipatory analysis, and systems thinking — is becoming a distinguishing capability of organisations that need to act earlier, think more clearly, and decide under uncertainty.
          </p>
          <p style={{ fontSize: 14, lineHeight: 1.85, color: '#64748b', marginBottom: 14 }}>
            Most organisations still treat risk management, intelligence, and data capability as separate functions. The real opportunity lies in their integration: building the decision infrastructure that helps organisations see further, reason more rigorously, and act with greater confidence under uncertainty. This is still an emerging capability space, shaped in practice by the teams and leaders working at that intersection.
          </p>
          <p style={{ fontSize: 14, lineHeight: 1.85, color: '#64748b', marginBottom: 14 }}>
            The through-line of this career has been the steady accumulation of capabilities needed for that work. Command decisions at operational tempo. Intelligence operations that turned raw signal into foresight. Data platforms that embedded analytical capability at scale. Anticipatory risk models that moved decision-making from reactive to forward-looking. Each chapter added a layer; the convergence is the point at which those layers begin to operate as one capability.
          </p>
          <p style={{ fontSize: 14, lineHeight: 1.85, color: '#475569', fontWeight: 600 }}>
            The next chapter is about leading where organisations are trying to make earlier, better, and more informed decisions in complex technical environments — across the teams and functions shaping how intelligent decision-making works in practice.
          </p>
        </div>
      </section>
